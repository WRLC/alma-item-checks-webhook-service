...
class DataService:
    ...
    def create_safe_filename(
        self, input_string: str, replacement: str = "_", max_len: int = 200
    ) -> str:
        """
    ...
    def serialize_data(self, data: Union[Dict, List]) -> str:
        ...
        try:
            ...
            logging.error(
                f"Failed to serialize data to JSON: {e}. Data type: {type(data)}"
            )
    ...
    def deserialize_data(self, data_string: Union[str, bytes]) -> Union[Dict, List]:
        ...
        if isinstance(data_string, bytes):
            try:
                ...
                raise DataSerializationError(
                    f"Failed to decode bytes using utf-8: {e}"
                ) from e

        if not isinstance(data_string, str):
            raise DataSerializationError(
                f"Input must be string or bytes, got {type(data_string)}"
            )
        ...
        try:
            ...
            logging.error(
                f"Failed to decode JSON data string: {e}. String starts with: '{data_string[:100]}'"
            )
            raise DataSerializationError(
                f"Failed to decode JSON data string: {e}"
            ) from e
